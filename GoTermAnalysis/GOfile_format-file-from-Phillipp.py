...
df_raw = pd.read_csv("./INPUT2_GO-mapping/GO-Data-Phillipp.txt", sep="\t")
...
for GO_category in data_all_GOcategories:
    ...
    df.to_csv(f"./INPUT2_GO-mapping/output_{GO_category}_terms_PHILLIPP.txt", sep="\t", index=False)
...
df_UniProt.to_csv("./INPUT2_GO-mapping/output_symbols_PHILLIPP.txt", sep="\t", index=False)
